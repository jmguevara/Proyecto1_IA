...
# where 0 means B and 1 means M
def clean(csv_file_name):
	df = pd.read_csv(csv_file_name, header=None, index_col=None)
...
def random_data_sets():
        ...
        x=st.get_z_matrix()#delete headers
...
def trainingForest(sub_data):
        ...
        for index in range (0,5):
                df = pd.DataFrame(sub_data[index])
                
                df.to_csv('training.csv', header=None, index=None)
                
                df_data=clean('training.csv')
               
                print(index)
                print(headers[1:])
                root = build_tree(df_data, headers[1:], 'Outcome')
